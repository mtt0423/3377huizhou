...
namespace DialogSystem
{
    ...
    {
        public string type;       // CSV第1列：标志（@/#/￥/&/END）
...
// ==================== 核心管理脚本（类名：ScenesoneDialogManager）====================
public class ScenesoneDialogManager : MonoBehaviour
{
    // ==== 基础配置（拖拽赋值）====
    public TextAsset dialogDataFile;       // CSV文件（赋值新的“朱熹3.0.CSV”）
    ...
    // ==== 可视化配置（Inspector中设置）====
    public List<DialogSystem.ItemConfig> itemConfigs = new List<DialogSystem.ItemConfig>();
    public List<DialogSystem.OptionConfig> optionConfigs = new List<DialogSystem.OptionConfig>();
    public List<DialogSystem.BackgroundConfig> backgroundConfigs = new List<DialogSystem.BackgroundConfig>();
    ...
    // ==== 内部变量（关键修正：分支触发ID改为62）====
    private List<DialogSystem.DialogData> dialogList = new List<DialogSystem.DialogData>();
    private int currentDialogID = 48;      // 新CSV初始剧情ID（保持48）
    private int triggerBranchDialogId = 62; // 分支触发剧情ID（修正为62）
    ...
    private void ReadCSV()
    {
        ...
        {
            Debug.LogError("❌ 未赋值CSV文件！请拖拽新的“朱熹3.0.CSV”到dialogDataFile字段");
            return;
        }
        ...
        while (reader.Peek() != -1)
        {
            ...
            DialogSystem.DialogData data = new DialogSystem.DialogData
            {
                type = columns[0].Trim(),
            ...
            if (data.id != -1)
            {
                ...
                if (data.type == "&")
                {
                    Debug.Log($"✅ 读取新CSV分支选项：ID={data.id}，文本={data.content}，跳转={data.nextId}");
                }
            }
        }
        Debug.Log($"✅ 新CSV读取完成，共{dialogList.Count}条剧情，其中分支选项{dialogList.FindAll(d => d.type == "&").Count}个");
    }
    ...
    public void ShowCurrentDialog(int dialogId)
    {
        ...
        DialogSystem.DialogData currentDialog = dialogList.Find(d => d.id == dialogId);
        if (currentDialog == null)
        {
            dialogContentText.text = "未找到对应剧情";
            Debug.LogError($"❌ 未找到ID为{dialogId}的剧情（检查新CSV是否有该ID）");
            return;
        }
        ...
        switch (currentDialog.type)
        {
            ...
            case "#": // 对话
                ...
                // 关键：触发分支的ID改为62，生成分支选项
                if (currentDialog.id == triggerBranchDialogId)
                {
                    GenerateOptions(); // 生成新CSV中的5个分支选项
        ...
        UpdateBackground(currentDialog.extProp); // 适配新CSV的背景配置
    }
    ...
    private void GenerateOptions()
    {
        ...
        // 读取新CSV中所有&类型分支选项
        List<DialogSystem.DialogData> allOptions = dialogList.FindAll(d => d.type == "&");
        Debug.Log($"🔍 新CSV找到{allOptions.Count}个分支选项");

        // 遍历生成所有选项按钮
        for (int i = 0; i < allOptions.Count; i++)
        {
            DialogSystem.DialogData option = allOptions[i];
            ...
            if (btnRect != null)
            {
                ...
                // 位置：优先配置，无则自动排列
                DialogSystem.OptionConfig targetConfig = optionConfigs.Find(config => config.optionText.Trim() == optionText);
            ...
            if (btn != null)
            {
                ...
                colorBlock.fadeDuration = colorTransitionDuration;
        ...
        Debug.Log($"📋 分支选项生成完成：{allOptions.Count}个按钮");
    }

    // ==================== 辅助方法（全部适配新CSV）====================
    private void ShowDialogContent(DialogSystem.DialogData dialog)
    {
    ...
    private void UpdateBackground(string extProp)
    {
        ...
        DialogSystem.BackgroundConfig bgConfig = backgroundConfigs.Find(bg => bg.backgroundName.Trim() == backgroundName);
        ...
        {
            Debug.LogWarning($"⚠️ 未找到背景配置：{backgroundName}（检查新CSV的extProp字段）");
        }
    }

    private void ShowCharacters(DialogSystem.DialogData dialog)
    {
    ...
    private void ShowName(DialogSystem.DialogData dialog)
    {
        if (nameText == null)
        {
    ...
    private void CreateClickableItem(string itemName, int nextId)
    {
        ...
        DialogSystem.ItemConfig targetConfig = itemConfigs.Find(config => config.itemName.Trim() == itemName);
        if (targetConfig == null)
        {
            Debug.LogError($"❌ 未找到物品「{itemName}」的配置（检查新CSV的extProp字段）");
            return;
        }
    ...
    // ==================== 继续按钮点击事件（适配新CSV的跳转ID）====================
    public void OnNextButtonClick()
    {
        DialogSystem.DialogData currentDialog = dialogList.Find(d => d.id == currentDialogID);
        if (currentDialog == null)
        {
        ...
        {
            DialogSystem.DialogData nextDialog = dialogList.Find(d => d.id == nextId);
            if (nextDialog != null)
            {
                ShowCurrentDialog(nextId);
            }
            ...
            {
                Debug.LogError($"❌ 未找到跳转目标剧情（ID：{nextId}，检查新CSV的nextId字段）");
            }
        }
        else
        {
